  ...
  return (
    <div className="max-w-md mx-auto bg-gradient-to-r from-gray-100 via-gray-200 to-gray-300 text-gray-800 rounded-lg shadow-lg p-8 my-20">
      ...
      <form onSubmit={handleNewsletterSubmit} className="flex flex-col space-y-4">
        ...
        />
        <button type="submit" className="bg-blue-600 text-white font-semibold p-3 rounded-md hover:bg-blue-700 transition duration-300">
          Subscribe
        </button>
        {message && (
          <p className={`text-center font-semibold ${message.includes('successfully') ? 'text-green-500' : 'text-red-500'}`}>
            {message}
          </p>
        )}
      </form>
